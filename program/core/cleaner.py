"""
Text Cleaning and Preprocessing for CommentCourt.

This module provides text cleaning utilities for Turkish e-commerce comments.
Handles:
- Normalization (lowercasing, whitespace)
- Noise removal (URLs, emails, special characters)
...
class TextCleaner:
    ...
    Provides configurable text preprocessing with Turkish language support.
    """
...
class CommentCleaner(TextCleaner):
    ...
    Extends TextCleaner with comment-specific preprocessing.
    """
    ...
    def clean(self, text: str) -> str:
        ...
        Extends base cleaning with comment-specific processing.
        """
        if not text:
            return ""
...
def clean_text(text: str, **kwargs) -> str:
    """
    Clean a single text with custom options.
