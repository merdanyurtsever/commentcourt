...
file_name = "database/raw/Veri_Seti.xlsx"
...
for epoch in range(EPOCHS):
    ...
    # 🔵 TRAIN METRICS (THRESHOLD = 0.670)
    train_preds = np.array(train_preds)
    train_truths = np.array(train_truths)

    train_acc = accuracy_score(
        (train_truths > THRESHOLD).astype(int),
        (train_preds > THRESHOLD).astype(int)
    )
    ...
    val_acc = accuracy_score(
        (val_truths > THRESHOLD).astype(int),
        (val_preds > THRESHOLD).astype(int)
    )
...
# 10. MODELİ KAYDET
# ==============================
SAVE_PATH = "sentiment_model_xlmr_final_threshold"
os.makedirs(SAVE_PATH, exist_ok=True)

model.save_pretrained(SAVE_PATH)
tokenizer.save_pretrained(SAVE_PATH)

# Threshold'u da ayrıca kaydetmek istersen:
with open(os.path.join(SAVE_PATH, "threshold.txt"), "w") as f:
    f.write(str(THRESHOLD))

print("\nXLM-R modeli, tokenizer ve threshold başarıyla kaydedildi.")